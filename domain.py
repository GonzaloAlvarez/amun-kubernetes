...
DESCRIPTION = (
    "ensure pfSense Unbound has a Domain Override pointing k8s.lan at our "
    "k8s_gateway LoadBalancer IP"
)
...
def _cleanup_venv() -> None:
    """Tear the venv down on exit. The python binary keeps running from RAM
    after the directory is removed (POSIX unlink semantics)."""
    try:
        shutil.rmtree(_VENV, ignore_errors=True)
    except Exception:
        pass
...
def detect_lb_ip() -> str:
    """Resolve the k8s_gateway LoadBalancer IP via kubectl."""
    try:
        out = subprocess.check_output(
            ["kubectl", "-n", "kube-system", "get", "svc", "k8s-gateway",
        ...
        pass
    sys.exit("could not detect k8s_gateway LB IP via kubectl; pass --ip explicitly.")
...
def csrf(session: requests.Session, url: str) -> str:
    ...
    r.raise_for_status()
    # tolerate either attribute order, single or double quotes
...
def get_overrides(session: requests.Session, base: str) -> list[dict]:
    ...
    Returns dicts with keys: idx (int), domain (str), ip (str).
    The pfSense UI lists overrides in a table; each row links to
    services_unbound_domainoverride_edit.php?id=N.
    """
    ...
    # Some pfSense skins put domain/ip BEFORE the edit link. Try a fallback parse.
    if not rows:
        alt = re.compile(
...
def upsert_override(session: requests.Session, base: str, *,
                    ...
                    descr: str = "k8s_gateway (managed by amun-kubernetes/domain.py)") -> None:
    ...
    # Probe DNS health before/after Apply Changes — pfSense regenerates
    # unbound.conf and restarts Unbound on Apply, and any pre-existing syntax
    # error in "Display Custom Options" stops Unbound from coming back. We
    # want to flag that loudly rather than silently succeed.
    ...
    # Wait for Unbound to come back. Initial sleep covers the typical reload
    # window; then poll with backoff in case it's slow.
    if not pre_ok:
    ...
    sys.exit(
        ...
        f"\n"
        f"   Common cause: a malformed line in pfSense → Services → DNS Resolver →\n"
        f"   Display Custom Options (e.g. `server: log-queries: yes` on one line\n"
        f"   instead of two — the `server:` header and the directive must be split).\n"
        f"\n"
        f"   To recover:\n"
        f"     1) https://{pf_host} → Status → Services → check `unbound` (likely Stopped)\n"
        f"     2) Diagnostics → Edit File → /var/unbound/unbound.conf to find the bad line\n"
        f"     3) Or: Services → DNS Resolver → Domain Overrides → trash the {domain}\n"
        f"        entry → Save → Apply (rolls back this script's change)\n"
    )
...
def _probe_dns_socket(host: str, timeout: float = 2.0) -> bool:
    """Send a minimal DNS query for the root zone to host:53/UDP. Return True on
    any reply with a matching transaction ID — even NXDOMAIN/REFUSED count as
    'Unbound is alive'."""
    txid = secrets.token_bytes(2)
    flags = b"\x01\x00"  # standard query, recursion desired
    counts = b"\x00\x01\x00\x00\x00\x00\x00\x00"  # 1 question
    qname = b"\x00"  # root (empty label)
...
def probe_dns(host: str, timeout: float = 2.0) -> bool:
    """Is host:53 answering at all? Tries `dig` (battle-tested), falls back to a
    raw socket query if dig isn't installed."""
    try:
        rc = subprocess.run(
...
def main() -> int:
    ...
    p.add_argument("--domain", default="k8s.lan",
                   help="DNS zone delegated to k8s_gateway (default: k8s.lan)")
    p.add_argument("--ip", default=None,
                   help="target IP (default: kubectl get svc k8s-gateway)")
    ...
    # Resolve the pfSense host with this precedence:
    # 1. --host CLI flag
    # 2. cached host in .pf-creds
    # 3. interactive prompt (default = detected default gateway)
    ...
    target_ip = args.ip or detect_lb_ip()
    base = f"https://{host}"
    print(f"pfSense: {base} | desired: {args.domain} → {target_ip}")

    session = requests.Session()
    session.headers.update({"User-Agent": "amun-kubernetes/domain.py"})
    ...
    if existing and existing["ip"] == target_ip:
        print(f"✓ {args.domain} already → {target_ip} (no change)")
        return 0

    upsert_override(session, base,
                    domain=args.domain, ip=target_ip,
                    idx=existing["idx"] if existing else None)
    verb = "updated" if existing else "created"
    print(f"✓ {args.domain} → {target_ip} ({verb})")
    return 0
